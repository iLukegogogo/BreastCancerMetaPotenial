...
HES4.expr  <- TCGA.breast.cancer.log2.fpkm.matrix[HES4,pure.TCGA.breast.cancer.polyA.Basal.sample] %>% sort
...
l.expr.matrix <- TCGA.breast.cancer.log2.fpkm.matrix[,l.sample]
h.expr.matrix <- TCGA.breast.cancer.log2.fpkm.matrix[,h.sample]
...
expr.matrix    <- cbind(TCGA.breast.cancer.log2.read.count.matrix[expressed.gene,h.sample],TCGA.breast.cancer.log2.read.count.matrix[expressed.gene,l.sample])
...
pam50.gene.expr             <- cbind(TCGA.breast.cancer.log2.fpkm.matrix[pam50.gene.df$probe %>% as.character,TCGA.breast.cancer.polyA.sample],
                                     SRP157974_PrimaryTumor_log2.fpkm.matrix[pam50.gene.df$probe %>% as.character,]
) %>% t
...
r  <- apply(rs$correlation.matrix,1,function(x) rank(x)['HCC1569_BREAST'])
plot(sort(r))
...
SRP157974_PrimaryTumor_log2.fpkm.matrix       <- SRP157974_PrimaryTumor_log2.fpkm.matrix[,basal.sample]
SRP157974_PrimaryTumor_log2.read.count.matrix <- SRP157974_PrimaryTumor_log2.read.count.matrix[,basal.sample]


HES4.expr <- SRP157974_PrimaryTumor_log2.fpkm.matrix[HES4,] %>% sort
...
l.expr.matrix <- SRP157974_PrimaryTumor_log2.read.count.matrix[,l.sample]
h.expr.matrix <- SRP157974_PrimaryTumor_log2.read.count.matrix[,h.sample]
...
expr.matrix    <- cbind(SRP157974_PrimaryTumor_log2.read.count.matrix[expressed.gene,h.sample],SRP157974_PrimaryTumor_log2.read.count.matrix[expressed.gene,l.sample])
expr.matrix    <- 2^expr.matrix - 1
df             <- data.frame(condition=c(rep(x='HES4.high',times=length(h.sample)), rep(x='HES4.low',times=length(l.sample))))
df$condition   <- factor(df$condition,levels = c('HES4.low','HES4.high'))
dds            <- DESeqDataSetFromMatrix(countData = round(expr.matrix),
...
# 
# HES4.expr     <- log2.fpkm.matrix[HES4,pure.TCGA.breast.cancer.polyA.Basal.sample] %>% sort
# l.sample      <- names(HES4.expr)[HES4.expr <  3]
# h.sample      <- names(HES4.expr)[HES4.expr >=  3]
...
# 
# 
# df.1 <- data.frame(HES4=log2.fpkm.matrix[HES4,l.sample],NOTCH3=log2.fpkm.matrix[NOTCH3,l.sample],m=BRCA.mutation[l.sample,'NOTCH3'])
# df.2 <- data.frame(HES4=log2.fpkm.matrix[HES4,h.sample],NOTCH3=log2.fpkm.matrix[NOTCH3,h.sample],m=BRCA.mutation[h.sample,'NOTCH3'])
...
#        )
# 
# df.1 <- data.frame(HES4=log2.fpkm.matrix[HES4,l.sample],NOTCH3=log2.fpkm.matrix[NOTCH3,l.sample],m=GISTIC.matrix['NOTCH3',l.sample],n=GISTIC.matrix['NOTCH1',l.sample],o=GISTIC.matrix['NOTCH4',l.sample])
# df.2 <- data.frame(HES4=log2.fpkm.matrix[HES4,h.sample],NOTCH3=log2.fpkm.matrix[NOTCH3,h.sample],m=GISTIC.matrix['NOTCH3',h.sample],n=GISTIC.matrix['NOTCH1',h.sample],o=GISTIC.matrix['NOTCH4',h.sample])
...
HES4.expr <- log2.fpkm.matrix[HES4,pure.TCGA.breast.cancer.polyA.LumB.sample] %>% sort
hist(HES4.expr,breaks=20)

l.sample <- names(HES4.expr)[HES4.expr <  2.5]
h.sample <- names(HES4.expr)[HES4.expr >=  2.5]
l.expr.matrix <- log2.fpkm.matrix[,l.sample]
h.expr.matrix <- log2.fpkm.matrix[,h.sample]
...
HES4.expr <- log2.fpkm.matrix[HES4,pure.TCGA.breast.cancer.polyA.Her2.sample] %>% sort
hist(HES4.expr,breaks=20)
quantile(HES4.expr,probs = seq(0, 1, 0.05))  %>% plot
...
HES4.expr <- log2.fpkm.matrix[HES4,pure.TCGA.breast.cancer.polyA.LumA.sample] %>% sort
hist(HES4.expr,breaks=40)
...
combined.expr.matrix             <- log2.fpkm.matrix[common.genes,c(l.sample,h.sample)]
...
# 
# s <- intersect(names(tumor.purity.based.on.cell.line.vec),colnames(log2.fpkm.matrix))
# plot(x=tumor.purity.based.on.cell.line.vec[s],y=log2.fpkm.matrix[HES4,s])
# 
# 
# cor.matrix     <- cor(x=log2.fpkm.matrix[HES4,],y=log2.fpkm.matrix %>% t,method='spearman')
# cor.vec        <- c(cor.matrix)
# names(cor.vec) <- colnames(cor.matrix)
# cor.vec        <- sort(cor.vec,decreasing = TRUE)
# 
# 
# HES4.expr <- log2.fpkm.matrix[HES4,] %>% sort
# 
...
# TCGA.breast.cancer.log2.read.count.matrix <- log2.read.count.matrix
# TCGA.breast.cancer.log2.fpkm.matrix       <- log2.fpkm.matrix
# TCGA.breast.cancer.log2.fpkm.matrix       <- TCGA.breast.cancer.log2.fpkm.matrix[,setdiff(colnames(TCGA.breast.cancer.log2.fpkm.matrix), TCGA.breast.cancer.polyA.Basal.sample)]
# 
# HES4            <- 'ENSG00000188290' # wow, bimodal distribution in Basal
# tmp             <- cor(TCGA.breast.cancer.log2.fpkm.matrix[HES4,], TCGA.breast.cancer.log2.fpkm.matrix %>% t, method='spearman')
...
# 
# 
# # TCGA.breast.cancer.log2.fpkm.matrix <- TCGA.breast.cancer.log2.fpkm.matrix[,order(TCGA.breast.cancer.log2.fpkm.matrix[HES4,])]
# # 
# # TCGA.breast.cancer.log2.fpkm.matrix <- TCGA.breast.cancer.log2.fpkm.matrix[,c(1:90,806:896)]
# # 
# # pca.rs <- prcomp(TCGA.breast.cancer.log2.fpkm.matrix %>% t)
# # plot(pca.rs$x[,1:2])
# 
# 
# #Basal.log2.fpkm.matrix <- TCGA.breast.cancer.log2.fpkm.matrix
# 
# #Basal.log2.fpkm.matrix  <-  TCGA.breast.cancer.log2.fpkm.matrix[,TCGA.breast.cancer.polyA.Her2.sample]
# #Basal.read.count.matrix <-  2^TCGA.breast.cancer.log2.read.count.matrix[,TCGA.breast.cancer.polyA.Her2.sample] - 1
# 
# 
# #Basal.log2.fpkm.matrix  <-  TCGA.breast.cancer.log2.fpkm.matrix[,TCGA.breast.cancer.polyA.LumB.sample]
# #Basal.read.count.matrix <-  2^TCGA.breast.cancer.log2.read.count.matrix[,TCGA.breast.cancer.polyA.LumB.sample] - 1
# 
# 
# HES4           <- 'ENSG00000188290' # wow, bimodal distribution in Basal
# df             <- data.frame(HES4.status = ifelse(TCGA.breast.cancer.log2.fpkm.matrix[HES4,] >=3, 'HES4.high','HES4.low'))
# df$HES4.status <- factor(df$HES4.status,levels = c('HES4.low','HES4.high'))
# flag           <- apply(TCGA.breast.cancer.log2.fpkm.matrix,1,function(x) median(x) > log2(1+0.1))
# TCGA.breast.cancer.log2.fpkm.matrix <- TCGA.breast.cancer.log2.fpkm.matrix[flag,]
# 
# 
# res <- foreach(g= rownames(TCGA.breast.cancer.log2.fpkm.matrix),.combine='rbind') %do% {
#     p.value     <- wilcox.test(TCGA.breast.cancer.log2.fpkm.matrix[g,df$HES4.status == 'HES4.high'],TCGA.breast.cancer.log2.fpkm.matrix[g,df$HES4.status != 'HES4.high'])$p.value 
#     effect.size <- median(TCGA.breast.cancer.log2.fpkm.matrix[g,df$HES4.status == 'HES4.high']) - median(TCGA.breast.cancer.log2.fpkm.matrix[g,df$HES4.status != 'HES4.high'])
#     data.frame(effect.size=effect.size,p.value=p.value)
# }
# rownames(res) <- rownames(TCGA.breast.cancer.log2.fpkm.matrix)
...
# 
# HES4           <- 'ENSG00000188290' # wow, bimodal distribution in Basal
# df             <- data.frame(HES4.status = ifelse(Basal.log2.fpkm.matrix[HES4,] >=3, 'HES4.high','HES4.low'))
# df$HES4.status <- factor(df$HES4.status,levels = c('HES4.low','HES4.high'))
# flag           <- apply(Basal.log2.fpkm.matrix,1,function(x) median(x) > log2(1+0.1))
...
# df$y <- Basal.log2.fpkm.matrix['ENSG00000168298',rownames(df)]
# df <- df[df$y < 2.5,]
