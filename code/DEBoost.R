...
perform.DE.analysis.between.TRE.and.CON <- function(CON.log2.tpm.matrix, CON.log2.read.count.matrix, TRE.log2.tpm.matrix,TRE.log2.read.count.matrix)
{
    ...
    dds           <- DESeqDataSetFromMatrix(countData = round(read.count.matrix),
                                            ...
                                            design = ~ condition)
    dds <- DESeq(dds)
    res <- results(dds,contrast = c('condition','TRE','CON')) %>% as.data.frame
...
DEBoost.filtering <- function(deseq2.M.vs.P.res, deseq2.R.vs.P.res, MET.log2.tpm.matrix, PRI.log2.tpm.matrix, REF.log2.tpm.matrix ){
    ...
    list(up.gene=c(up.gene),dn.gene=c(dn.gene),up.outlier.gene = up.outlier.gene,dn.outlier.gene = dn.outlier.gene)
...

{
  ...
  tumor.intrinsic.DE.gene.rs  <- DEBoost.filtering(deseq2.M.vs.P.res = deseq2.M.vs.P.res,deseq2.R.vs.P.res = deseq2.R.vs.P.res, MET.log2.tpm.matrix = MET.log2.tpm.matrix, PRI.log2.tpm.matrix = PRI.log2.tpm.matrix,REF.log2.tpm.matrix = REF.log2.tpm.matrix)
